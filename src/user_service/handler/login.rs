...
fn match_user_with_hash(
    ...
    match email_identities
        .filter(lower(email).eq(user_email))
        .filter(created_at.eq(updated_at))
        .order(created_at.desc())
        .select((hash, user_id))
        .first::<(String, Uuid)>(connection)
    {
        Ok((password_hash, user_uuid)) => {
            match_password_with_hash(password, password_hash, &user_uuid)
        }
        Err(e) => {
...
fn err() -> ResponseResult {
    Err(Status::unauthenticated(
        "Credentials don't match or user does not exist.",
    ))
}
